...
final class TerminalSurfaceCoordinator {
    ...
    private func performMetricsSync() -> Bool {
        #if DEBUG
            if let testHooks_metricsSyncResult {
                return testHooks_metricsSyncResult
            }
        #endif
        ...
        // Preserve cell dimensions for the correctly phased IO-thread resize
        // callback without dispatching a premature host resize from here.
        configuration.inMemorySession?.updateCellSize(
            widthPixels: surfaceSize.cellWidthPixels,
            heightPixels: surfaceSize.cellHeightPixels
        )
    ...
    #if DEBUG
        // Test access to the host-managed resize state. The result override
        // exercises throttle scheduling without requiring a real ghostty
        // surface.
